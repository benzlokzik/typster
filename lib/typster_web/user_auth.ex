defmodule TypsterWeb.UserAuth do
  ...
  @remember_me_cookie "_typster_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      TypsterWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  @doc """
  Authenticates JSON API requests from an Authorization bearer token.

  API clients should send the URL-safe base64 encoded value returned by
  ...
  @doc """
  Requires an authenticated API scope.
  """
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      TypsterWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule TypsterWeb.PageLive do
        use TypsterWeb, :live_view

        on_mount {TypsterWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{TypsterWeb.UserAuth, :require_authenticated}] do
